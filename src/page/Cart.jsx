...
function Cart() {
  ...
  const load = async () => {
    setLoading(true);
    try {
      const res = await fetch(`https://tech-store-2.onrender.com/api/cart/${cartId}`);
  ...
  const updateQty = async (productId, quantity) => {
    try {
      await fetch(`https://tech-store-2.onrender.com/api/cart/${cartId}/items/${productId}`, {
        method: "PUT",
  ...
  const removeItem = async (productId) => {
    try {
      await fetch(`https://tech-store-2.onrender.com/api/cart/${cartId}/items/${productId}`, { method: "DELETE" });
      await load();
    } catch (e) { console.error(e); }
  };
  ...
  const clearCart = async () => {
    try {
      await fetch(`https://tech-store-2.onrender.com/api/cart/${cartId}`, { method: "DELETE" });
      await load();
    } catch (e) { console.error(e); }
  };
  ...
  const placeOrder = async () => {
    ...
    try {
      const res = await fetch("https://tech-store-2.onrender.com/api/orders", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
